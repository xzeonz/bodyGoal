    ...
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white to-gray-100 px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-lg space-y-3">
          ...
          {error && (
            <div className="p-3 rounded-lg bg-red-50 border border-red-200">
              <p className="text-red-600 text-sm text-center">
                {error === 'missing-fields' && 'Email dan password harus diisi.'}
                {error === 'invalid-credentials' && 'Email atau password salah.'}
                {error === 'user-exists' && 'Email sudah terdaftar.'}
                {!['missing-fields', 'invalid-credentials', 'user-exists'].includes(error) && 'Terjadi kesalahan. Silakan coba lagi.'}
              </p>
            </div>
          )}
        ...
        <a
          ...
          className="w-full py-2 border border-gray-300 rounded-lg hover:bg-gray-100 flex items-center justify-center gap-2 transition block"
        >
